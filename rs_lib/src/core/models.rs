...
impl From<js_sys::Date> for DateLike {
    fn from(value: js_sys::Date) -> Self {
        let date = time::Date::from_calendar_date(
            value.get_full_year().try_into().unwrap(),
            // Time : month from 1 to 12
            // Js_sys : month from 0 to 11
            time::Month::January.nth_next(value.get_utc_month().try_into().unwrap()),
            // Js_sys : get_date for day in a month
            // get_day : day in a week
            value.get_date().try_into().unwrap(),
        )
        .unwrap();
        date.into()
    }
